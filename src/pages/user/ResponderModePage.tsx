    ...
    <div className="min-h-screen bg-[#070B14] py-8 px-4 sm:px-6 lg:px-8 text-slate-100">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
          ...
          <div className="flex items-center gap-3 glass-panel p-2 rounded-2xl border border-white/10">
            <span className="text-xs font-bold text-slate-300 ml-2">Status:</span>
            <button
              onClick={() => {
                if (!isResponder) {
        ...
        <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 space-y-4">
                ...
                >
                  ...
                  <div
                    className={`w-4 h-4 rounded flex items-center justify-center text-[10px] ${
                      isSelected ? 'bg-emerald-500 text-white' : 'border border-slate-600'
                    }`}
                  >
                    {isSelected && '✓'}
                  </div>
          ...
          <div className="pt-2 flex items-center justify-end">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider shadow-md transition-transform active:scale-95"
            >
              Save & Go Available
            </button>
          </div>
                ...
                >
                  ...
                  <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs text-slate-400">
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                        to={`/requests/${req.id}`}
                        className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-semibold transition-colors"
                      >
                        View
                      </Link>
                      <button
                        onClick={() => handleAcceptNearby(req.id)}
                        className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider shadow-sm transition-transform active:scale-95"
                      >
                        Accept
                      </button>
                    </div>
