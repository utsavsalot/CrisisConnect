    ...
    <aside className="hidden w-56 shrink-0 self-center lg:block xl:w-64" aria-label="Life-saving techniques">
      <div className={`border-red-500 ${showIntro ? 'border-l-2 pl-6' : 'border-r-2 pr-6'}`}>
        {showIntro && <>
          ...
          <p className="mt-5 text-sm font-medium leading-relaxed text-black/75">
            Simple actions can protect a life while verified help is moving toward you.
          </p>
          <figure className="mt-6 overflow-hidden rounded-2xl border border-black/10 bg-white shadow-[0_12px_28px_rgba(15,23,42,0.12)]">
        ...
        <div className={showIntro ? 'mt-8 space-y-6' : 'space-y-6'}>
          {safetyStories.slice(storyStart, storyStart + storyCount).map(([number, title, copy, Icon, image]) => (
            <div key={number} className="border-t border-black/15 pt-3">
              ...
              <p className="mt-1 text-xs leading-relaxed text-black/60">{copy}</p>
            </div>
          ))}
        </div>
