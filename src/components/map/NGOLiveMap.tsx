...
export const NGOLiveMap: React.FC<NGOLiveMapProps> = ({ requests, onAcceptRequest }) => {
  ...
  return (
    <div className="relative w-full h-[calc(100vh-8rem)] rounded-3xl overflow-hidden glass-panel border border-white/10 flex flex-col lg:flex-row shadow-2xl">
      ...
      <div className="relative flex-1 h-full min-h-[400px]">
        ...
        {!tileError ? (
          ...
          <div className="w-full h-full bg-[#070B14] relative flex items-center justify-center overflow-hidden">
            ...
            {filteredRequests.map((req, idx) => {
              ...
              return (
                <button
                  ...
                  className="absolute p-2 rounded-full bg-emergency-600/30 border border-emergency-500 text-white shadow-emergency-glow hover:scale-125 transition-transform"
                >
          ...
          {['ALL', 'Medical Assistance', 'Blood', 'Rescue', 'Medicine', 'Food', 'Shelter', 'Transportation'].map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wider uppercase whitespace-nowrap backdrop-blur-md border transition-all ${
                categoryFilter === cat
                  ? 'bg-emergency-600 border-emergency-500 text-white shadow-emergency-glow'
      ...
      <div className="w-full lg:w-96 border-t lg:border-t-0 lg:border-l border-white/10 bg-slate-900/70 backdrop-blur-2xl flex flex-col h-72 lg:h-full z-20">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          ...
          {selectedRequest && (
            <button
              ...
              className="p-1 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white text-xs"
            >
              <X className="w-4 h-4" />
            </button>
          )}
          ...
          <div className="p-4 overflow-y-auto space-y-4 flex-1 no-scrollbar animate-in fade-in duration-200">
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/10 text-xs space-y-1.5">
              <div className="flex items-center gap-2 text-slate-400">
                ...
                <span className="text-slate-200 truncate">{selectedRequest.location.address || 'Detected Location'}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-400">
            ...
            {selectedRequest.status === 'active' && (
              <button
                ...
                className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95"
              >
            ...
            {filteredRequests.length === 0 ? (
              ...
              filteredRequests.map((r) => (
                <div
                  ...
                  className="p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-sky-500/30 cursor-pointer transition-all text-left"
                >
