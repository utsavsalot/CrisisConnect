...
const RequestRow: React.FC<{ request: EmergencyRequest; selected: boolean; onSelect: () => void; onAccept: () => void }> = ({ request, selected, onSelect, onAccept }) => (
  <button onClick={onSelect} className={`w-full rounded-xl border p-4 text-left transition ${selected ? 'border-red-300 bg-red-50/60' : 'border-slate-200 bg-white hover:border-red-200 hover:bg-slate-50'}`}>
    <div className="flex items-start justify-between gap-3"><div className="flex flex-wrap gap-1.5">{request.needs.map((need) => <span key={need} className="rounded-full bg-red-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-red-700">{need}</span>)}</div><span className="shrink-0 font-mono text-[10px] text-slate-400">{request.distanceKm} km</span></div>
