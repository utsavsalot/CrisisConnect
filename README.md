<div align="center">

<img src="public/logo.png" alt="CrisisConnect Logo" width="120"/>

# 🚨 CrisisConnect

### Real-Time Emergency Assistance Response Platform

**Connecting people in crisis with the right help at the right time.**

<p>
  <img src="https://img.shields.io/badge/React-19.x-61DAFB?style=for-the-badge&logo=react&logoColor=black"/>
  <img src="https://img.shields.io/badge/TypeScript-5.x-3178C6?style=for-the-badge&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/Vite-7.x-646CFF?style=for-the-badge&logo=vite&logoColor=white"/>
  <img src="https://img.shields.io/badge/Tailwind_CSS-4.x-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white"/>
</p>

<p>
  <img src="https://img.shields.io/badge/Firebase-Auth%20%7C%20Firestore-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"/>
  <img src="https://img.shields.io/badge/Leaflet-Maps-199900?style=for-the-badge&logo=leaflet&logoColor=white"/>
  <img src="https://img.shields.io/badge/GSAP-Animations-88CE02?style=for-the-badge&logo=greensock&logoColor=black"/>
  <img src="https://img.shields.io/badge/Anime.js-Animations-FF0055?style=for-the-badge"/>
</p>

<br/>

<a href="YOUR_LIVE_DEMO_URL">
  <img src="https://img.shields.io/badge/🚀%20Live%20Demo-Visit%20Project-6366F1?style=for-the-badge"/>
</a>

<a href="YOUR_GITHUB_REPOSITORY_URL">
  <img src="https://img.shields.io/badge/💻%20GitHub-Repository-181717?style=for-the-badge&logo=github"/>
</a>

</div>

---

# 📌 Project Information

| Category | Details |
|---|---|
| **Project Name** | CrisisConnect |
| **Event** | IGNITE 8.0 |
| **Event Type** | 18-Hour Hackathon |
| **Organizer** | IT Department |
| **Institution** | SVKM's Shri Bhagubhai Mafatlal Polytechnic |
| **Category** | Emergency Response & Social Impact |
| **Application Type** | Full-Stack Web Application |
| **Primary Focus** | Real-Time Emergency Assistance & Coordination |

---

# 📖 Table of Contents

- [About CrisisConnect](#-about-crisisconnect)
- [Problem Statement](#-problem-statement)
- [Our Solution](#-our-solution)
- [Key Features](#-key-features)
- [Emergency Workflow](#-emergency-workflow)
- [Crisis Priority Engine](#-crisis-priority-engine)
- [Automatic Escalation System](#-automatic-escalation-system)
- [User Module](#-user-module)
- [Responder Module](#-responder-module)
- [NGO Module](#-ngo-module)
- [Maps & Location Services](#-maps--location-services)
- [Communication & Notifications](#-communication--notifications)
- [CrisisAI & Crisis Guide](#-crisisai--crisis-guide)
- [System Architecture](#-system-architecture)
- [Emergency Lifecycle](#-emergency-lifecycle)
- [Tech Stack](#-tech-stack)
- [Project Structure](#-project-structure)
- [Getting Started](#-getting-started)
- [Demo Workflow](#-demo-workflow)
- [Authentication & Access Control](#-authentication--access-control)
- [UI & UX](#-ui--ux)
- [Screenshots](#-screenshots)
- [Future Enhancements](#-future-enhancements)
- [Hackathon](#-hackathon)
- [Disclaimer](#-disclaimer)
- [License](#-license)

---

# 🚨 About CrisisConnect

**CrisisConnect** is a full-stack emergency assistance response platform designed to connect people in urgent need with nearby responders, NGOs, and available resources.

During emergencies, people often struggle to find the right assistance quickly. CrisisConnect provides a centralized platform where emergency requests can be created, prioritized, matched with responders, tracked, and resolved.

The platform supports emergency needs such as:

- 🩸 Blood
- 💊 Medicine
- 🏥 Medical Assistance
- 🍱 Food
- 🏠 Shelter
- 🚗 Transportation
- 🚑 Rescue
- 💧 Water
- 🆘 Other Emergency Assistance

The system combines emergency prioritization, location-based assistance, responder availability, NGO resources, communication, notifications, and escalation into a single platform.

---

# ❗ Problem Statement

During emergencies and disasters, assistance can become difficult to coordinate because:

- People may not know where to request help.
- Emergency requests can remain unnoticed.
- Available responders may not know about nearby requests.
- NGOs may have resources but lack visibility into active needs.
- Emergency situations can become more critical while waiting.
- Multiple organizations may respond to the same request.
- Communication between victims and responders can become fragmented.
- Resource availability can change rapidly.

A system is needed to improve the coordination between:

**People → Responders → NGOs → Emergency Resources**

---

# 💡 Our Solution

CrisisConnect creates a unified emergency response ecosystem.

A person can create an emergency request with:

- Emergency category
- Description
- Current location
- Number of people affected
- Medical severity
- Additional information

The system then processes the request through the **Crisis Priority Engine** and makes it available to appropriate responders and NGOs.

Responders can:

- Enable responder mode
- View nearby emergency requests
- Filter requests
- Accept requests
- Navigate to the emergency location
- Communicate with the requester
- Complete the assistance

NGOs can:

- Monitor emergency requests
- Accept/claim emergencies
- Manage resources
- Track active assistance
- View emergencies on a live map

---

# ⭐ Key Features

## 🚨 Emergency Response

- One-Tap SOS Request
- Emergency Category Selection
- Live Location Detection
- Emergency Description
- People Affected Counter
- Medical Severity Selection
- Emergency Request Tracking
- Request Timeline
- Emergency Status Tracking

## 🧠 Intelligent Prioritization

- Crisis Priority Engine
- Priority Score
- Priority Levels
- Priority Breakdown
- Waiting-Time Priority Increase
- Resource Scarcity Analysis
- Priority-Based Response Queue
- What-If Analysis

## 📢 Automatic Escalation

- Local Emergency Broadcast
- Expanded Search Radius
- Priority Escalation
- Admin Escalation
- Escalation History

## 👤 User Features

- User Dashboard
- Emergency Requests
- Request History
- Request Tracking
- Notifications
- Profile Management
- Emergency Contacts
- Medical Information
- Settings

## 🦺 Responder Features

- Responder Mode ON/OFF
- Availability Status
- Responder Capabilities
- Nearby Emergency Feed
- Emergency Category Filters
- Accept Emergency Requests
- Emergency Navigation
- Communication Workflow

## 🏢 NGO Features

- NGO Dashboard
- Emergency Request Queue
- Emergency Request Details
- Accept / Claim Emergency
- Active Assistance Management
- NGO Live Map
- Resource Management
- Resource Availability Tracking
- Resource Updates
- Emergency Resource Categories

## 🗺️ Location & Maps

- Live Location Detection
- Emergency Location Map
- NGO Live Emergency Map
- Distance Information
- Google Maps Navigation
- Location-Based Emergency Discovery

## 💬 Communication

- Real-Time Emergency Chat
- Responder Communication
- NGO Communication
- System Messages
- Request Status Notifications
- Message Notifications
- System Alerts

## 🤖 CrisisAI

- Emergency Safety Guidance
- Crisis Assistance
- Safety Recommendations
- Emergency Information Support

## 📚 Crisis Guide

- First Aid
- Fire Safety
- Earthquake Safety
- Flood Safety
- Emergency Preparedness
- Finding Shelter
- Food & Water Safety
- Personal Safety
- Emergency Contacts
- Crisis Guide Search

---

# 🔄 Emergency Workflow

```text
User Needs Help
      │
      ▼
Create Emergency Request
      │
      ▼
Location Detection
      │
      ▼
Emergency Information
      │
      ├── Category
      ├── Severity
      ├── People Affected
      └── Description
      │
      ▼
Crisis Priority Engine
      │
      ▼
Priority Assignment
      │
      ▼
Nearby Responder / NGO Matching
      │
      ▼
Request Accepted
      │
      ▼
Communication
      │
      ▼
Navigation & Assistance
      │
      ▼
Request Resolved

🏢 NGO Module

NGOs have a dedicated dashboard for managing emergency response operations.

NGO Dashboard

NGOs can:

View emergency requests
Accept/claim requests
Monitor active assistance
View emergency locations
Manage available resources
Track resource quantities
Update resource availability
NGO Resource Management

Resources can be managed according to:

Resource Type
Total Quantity
Available Quantity
Allocated Quantity
Unit
Updated Time

This allows NGOs to maintain visibility over resources that can be used during emergencies.

🗺️ Maps & Location Services

Location plays an important role in emergency response.

CrisisConnect provides:

Live location detection
Emergency location mapping
NGO emergency maps
Distance information
Location-based emergency discovery
Navigation to emergency locations

The application uses Leaflet / React-Leaflet for map visualization.

Responders can also use navigation to reach the emergency location.

💬 Communication & Notifications

Emergency coordination requires communication between the requester and responder.

CrisisConnect provides an emergency communication workflow including:

Requester ↔ Responder communication
Requester ↔ NGO communication
Emergency chat
System messages
Request acceptance notifications
Status notifications
Message notifications
System alerts

The notification system keeps users informed about important changes in their emergency requests.

🤖 CrisisAI & Crisis Guide
CrisisAI

CrisisConnect includes a CrisisAI Safety Assistant that provides emergency-related guidance and safety information.

It is designed to help users understand what they can do during emergency situations while they seek appropriate assistance.

📚 Crisis Guide

The Crisis Guide provides structured emergency information covering:

How to Use CrisisConnect
What to Do During an Emergency
First Aid
Emergency Preparedness
Fire Safety
Earthquake Safety
Flood Safety
Finding Shelter
Food & Water Safety
Personal Safety
Emergency Contacts

The guide also supports topic navigation and search.

🏗️ System Architecture
                         ┌──────────────────────┐
                         │      CrisisConnect   │
                         │      Web Platform    │
                         └──────────┬───────────┘
                                    │
                                    ▼
                         ┌──────────────────────┐
                         │   React + TypeScript │
                         │       + Vite         │
                         └──────────┬───────────┘
                                    │
             ┌──────────────────────┼──────────────────────┐
             │                      │                      │
             ▼                      ▼                      ▼
       ┌───────────┐        ┌──────────────┐       ┌─────────────┐
       │   User    │        │   Responder  │       │     NGO     │
       │   Module  │        │    Module    │       │   Module    │
       └─────┬─────┘        └──────┬───────┘       └──────┬──────┘
             │                     │                      │
             └─────────────────────┼──────────────────────┘
                                   │
                                   ▼
                         ┌──────────────────────┐
                         │ Application Services │
                         │     & Contexts      │
                         └──────────┬───────────┘
                                    │
                  ┌─────────────────┼─────────────────┐
                  │                 │                 │
                  ▼                 ▼                 ▼
           ┌────────────┐    ┌─────────────┐   ┌────────────┐
           │  Firebase  │    │ Crisis      │   │   Maps &   │
           │ Auth / DB  │    │ Priority    │   │ Location   │
           └────────────┘    │ Engine      │   └────────────┘
                             └─────────────┘
🔄 Emergency Lifecycle
CREATE
  ↓
VERIFY / PROCESS
  ↓
PRIORITIZE
  ↓
MATCH
  ↓
ACCEPT
  ↓
COMMUNICATE
  ↓
ASSIST
  ↓
TRACK
  ↓
RESOLVE

If the request remains unresolved:

WAITING
   ↓
LOCAL BROADCAST
   ↓
EXPANDED SEARCH
   ↓
PRIORITY ESCALATION
   ↓
ADMIN ALERT
🛠️ Tech Stack
Frontend
Technology	Purpose
React.js	UI development
TypeScript	Type-safe development
Vite	Development & build tooling
Tailwind CSS	Styling
shadcn/ui	UI components
React Router	Application routing
React Hook Form	Form management
Zod	Validation
GSAP	Advanced animations
Anime.js	UI animations
Lucide Icons	Interface icons
Backend & Services
Technology	Purpose
Firebase Authentication	Authentication
Firebase Firestore	Application data
Firebase Storage	File/storage support
Firebase Services	Backend infrastructure
Maps
Technology	Purpose
Leaflet	Interactive maps
React-Leaflet	React map integration
Location Services	Emergency location detection
📁 Project Structure
CrisisConnect-main/
│
├── public/
│   └── logo.png
│
├── src/
│   │
│   ├── components/
│   │   ├── animations/
│   │   ├── chatbot/
│   │   ├── emergency/
│   │   ├── landing/
│   │   ├── map/
│   │   ├── navigation/
│   │   └── ui/
│   │
│   ├── context/
│   │   ├── AuthContext.tsx
│   │   ├── ChatContext.tsx
│   │   ├── EmergencyContext.tsx
│   │   └── ThemeContext.tsx
│   │
│   ├── data/
│   │   └── crisisGuideData.ts
│   │
│   ├── layouts/
│   │   └── RootLayout.tsx
│   │
│   ├── ngo-portal/
│   │   ├── NGOPortalApp.tsx
│   │   └── main.tsx
│   │
│   ├── pages/
│   │   ├── admin/
│   │   ├── ngo/
│   │   ├── public/
│   │   └── user/
│   │
│   ├── routes/
│   │   └── AppRoutes.tsx
│   │
│   ├── services/
│   │   ├── emergency/
│   │   ├── firebase/
│   │   └── mock/
│   │
│   ├── types/
│   │   └── index.ts
│   │
│   ├── App.tsx
│   ├── index.css
│   └── main.tsx
│
├── firestore.rules
├── package.json
├── vite.config.ts
├── tailwind.config.ts
├── tsconfig.json
└── README.md
